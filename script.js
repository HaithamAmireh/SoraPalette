...
document.getElementById('uploadButton').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch("https://sorapalette-backend.haithamamireh.com/process-image/", {
            method: "POST",
            body: formData
        });
        ...
        if (!colorData.colors || !Array.isArray(colorData.colors) || colorData.colors.length === 0) {
